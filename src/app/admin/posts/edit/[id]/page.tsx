...
export default function EditPost({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setFormData({
        ...
        content: 'فلسطین کی تاریخ بہت قدیم ہے اور اس میں بہت سے اہم واقعات شامل ہیں جو ہمارے لیے سبق آموز ہیں۔ یہاں ہم فلسطین کی تاریخ کے اہم مواقع کا جائزہ لیتے ہیں جو آج کے دور کے حالات کو سمجھنے میں مددگار ثابت ہو سکتا ہے۔',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('پوسٹ کامیابی سے اپ ڈیٹ کر دی گئی!');
      router.push('/admin/posts');
    } catch (err) {
      setError('پوسٹ اپ ڈیٹ کرنے میں ناکامی ہوئی۔ براہ کرم دوبارہ کوشش کریں۔');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6 text-palestine-green">پوسٹ تدوین کریں</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md dark:bg-gray-800">
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            {submitting ? 'اپ ڈیٹ ہو رہا ہے...' : 'اپ ڈیٹ کریں'}
          </button>
        </div>
      </form>
    </div>
